...
**翠玉AI导览** 是一款针对现代智慧景区深度定制的**双端智能服务系统**。它完美融合了移动互联网、大语言模型与地理信息技术，为游客与景区管理者提供一站式、个性化的交互体验。

*   **游客端（C端）**：以 7×24 小时在线的 **AI 数字人“小玉”** 为核心，提供个性化的景点语音讲解、智能 RAG 问答、定制化路线规划、历史足迹打卡以及适老化与童趣的多模式人机交互。
*   **管理端（B端）**：为景区运营方提供实时的**数据大屏**，支持知识库文档上传与分词标签管理、数字人形象与音色配置、游客交互情感倾向及满意度数据分析，助力景区实现精细化智慧运营。

系统基于 **Eazo 平台** 深度开发，无缝适配普通浏览器与 Eazo 移动端 App，具备强大的会话无感登录、用户偏好同步与系统级推送发布能力。
...
系统使用典型的**前后端解耦+云平台网关**架构。前端页面采用 Next.js App Router 分流，与后端 API 路由安全通信，AI 和安全认证则由 Eazo 平台和 AWS Bedrock 网关强力支持：

```mermaid
graph TD
    subgraph 客户端 (C端 & B端应用)
        Web[Web 浏览器 / Eazo Mobile WebView]
        EazoSDK[@eazo/sdk / useEazo]
    end

    subgraph 后端业务层 (Next.js Node 运行时)
        API[Next.js API 路由 /api/*]
        MCP[MCP 服务端 /api/mcp]
        Drizzle[Drizzle ORM 客户端]
    end

    subgraph 外部核心云服务
        EazoPlatform[Eazo 平台 - 统一 Auth & 语义记忆 Gum]
        Bedrock[AWS Bedrock - DeepSeek v3.1]
        Postgres[(PostgreSQL 托管数据库)]
    end

    Web -->|HTTP 请求 / EventStream| API
    Web -->|集成组件| EazoSDK
    EazoSDK -.->|会话校验 / 埋点上报| EazoPlatform
    API -->|requireAuth 安全校验| EazoPlatform
    API -->|drizzle-orm 读写| Drizzle
    API -->|bedrock-mantle 发起推理| Bedrock
    Drizzle -->|连接/流式回传| Postgres
    MCP -->|提供标准化智能体工具| Drizzle
```
...
#### 🙋 游客端 (C端)
*   **数字人互动主页**：首页展示动态数字人形象，模拟眨眼与说话动作。支持点击快速指令（如“适合老人路线”、“门票价格”），小玉自动发声讲解。
...
*   **代码总行数**：**8,659 行** (TypeScript + TSX + CSS，不包含依赖与打包结果)
*   **源码文件总数**：**109 个** 源文件
*   **页面及屏幕视图组件数**：**16 个** 核心视图模块
    *   **游客端视图 (9个)**：首页数字人交互、问答面板、景点广场、景点详情、推荐线路、线路详情、全局检索、个人中心、偏好配置。
...
### Q3：为什么我无法在浏览器客户端的代码中直接导入并配置 `@eazo/sdk` 的 `ai` 模块？
为了绝对保障开发者私钥（`EAZO_PRIVATE_KEY`）的安全，避免被反编译泄漏，`@eazo/sdk` 中的 `ai` 模块被设计为 **Server-Side Only**。所有的大模型交互必须在 Next.js 的服务端 Route Handlers 中进行，客户端组件只需通过 `fetch` 方式请求你的服务器端点即可。

### Q4：如何连接 Cursor 或 Claude Desktop 使用本项目提供的 MCP 智能体接口？
在配置文件中将项目的 `/api/mcp` 端点填入服务器配置中，并添加 `x-eazo-session` Header 签名即可。在本地开发时，配置示例如下：
```json
{
  "mcpServers": {
    "cuiyu-ai-guide": {
      ...
      "headers": {
        "x-eazo-session": "你的Eazo用户会话Token"
      }
    }
  }
}
```
这样 AI 助手便拥有了实时查询景区景点与路线的超能力。

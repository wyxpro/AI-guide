...
export class LAppSubdelegate {
  ...
  public onPointMoved(pageX: number, pageY: number): void {
    ...
    const localX: number = pageX - (rect.left + window.scrollX);
    const localY: number = pageY - (rect.top + window.scrollY);
    ...
    if (this._captured) {
      ...
      if (this._view) {
        ...
        const viewX = this._view.transformViewX(posX);
        const viewY = this._view.transformViewY(posY);

...
export function WeChatVoiceCallModal({
  ...
  // MediaRecorder Fallback for Mobile / Safari browsers
  const startMediaRecorderFallback = async () => {
    try {
      if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
        setLiveTranscript("当前浏览器暂不支持麦克风录音");
        return;
      }
      ...
      setLiveTranscript("未获取到麦克风权限，请在手机系统设置中开启");
    }
  };
  ...
  const startRecognition = () => {
    ...
    try {
      ...
      console.warn("Failed to start SpeechRecognition, falling back to MediaRecorder", err);
      startMediaRecorderFallback();
    }
  };
          ...
          <footer className="relative z-30 w-full max-w-md mx-auto pb-2 md:pb-3 px-4 flex-shrink-0">
            <div className="flex items-center justify-around">
              ...
              <div className="flex flex-col items-center gap-1.5">
                <button
                  onClick={toggleMute}
                  className={`w-13 h-13 sm:w-14 sm:h-14 rounded-full flex items-center justify-center transition-all active:scale-90 border shadow-lg ${
                    isMuted
              ...
              <div className="flex flex-col items-center gap-1.5">
                <button
                  ...
                  className="w-15 h-15 sm:w-16 sm:h-16 rounded-full bg-gradient-to-tr from-red-600 to-rose-500 flex items-center justify-center text-white shadow-[0_10px_25px_rgba(225,29,72,0.5)] transition-all active:scale-90 hover:brightness-110"
                >
